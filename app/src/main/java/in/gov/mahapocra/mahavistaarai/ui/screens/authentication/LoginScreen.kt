...
//            setPreventControlMeasureWebView(preventiveMeasures) //sets what happens
    ...
    private fun sendOTP() {
        ...
        loginViewModel.getOTPRequest(this@LoginScreen, mobile.trim())
    }
    ...
    private fun callLoginAPI(strToken: String, otp: String = "") {
        if (mobileNo.isEmpty()) {
            ...
            if (otp != "") {
                loginViewModel.getUserLoginOTP(this, mobileNo, otp, strToken)
            } else {
                loginViewModel.getUserLoginPassword(
                    this,
                    mobileNo.trim { it <= ' ' },
                    toSHA512(userPass),
                    strToken
        ...
        if (i == 4) {
            if (jSONObject != null) {
                if (jSONObject.optInt("status") == 200) {
                    ...
                    if (agriStackMobile != "") {
                        loginViewModel.getUserLoginOTP(this, mobileNo, enteredOTP, refreshToken)
                    } else {
